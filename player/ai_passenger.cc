...
{
	const vehikel_desc_t *v_desc = vehikel_search(water_wt, 10, 40, warenbauer_t::passagiere, false);
	if(v_desc==NULL  ) {
		// no ship there
		return false;
	}
	...
	{
		// we use the free own vehikel_desc_t
		vehikel_desc_t remover_desc( water_wt, 500, vehikel_desc_t::diesel );
	...
	if(!start_hub.is_bound()) {
		...
		if(town_road!=bushalt) {
			...
			bauigel.init_builder(way_builder_t::strasse, way_builder_t::weg_search(road_wt, road_vehicle->get_geschw(), road_vehicle->get_weight(), welt->get_timeline_year_month(), type_flat, 1), tunnel_builder_t::get_tunnel_desc(road_wt,road_vehicle->get_geschw(),welt->get_timeline_year_month()), NULL );
...
{
	const vehikel_desc_t *v_desc = vehikel_search(air_wt, 10, 900, warenbauer_t::passagiere, false);
	if(v_desc==NULL) {
		// no aircraft there
		return false;
	}
...
//				road_weg = way_builder_t::weg_search( road_wt, road_vehicle->get_geschw(), welt->get_timeline_year_month(),type_flat );
				...
				road_weg = way_builder_t::weg_search(road_wt, road_vehicle->get_geschw(), road_vehicle->get_weight(), welt->get_timeline_year_month(), type_flat, 1);
				...
				if(line->get_finance_history(0,LINE_PROFIT)<0) {
					...
					if(welt->use_timeline()  &&  line->count_convoys()>1) {
						...
						if(capacity>0) {
							...
							vehikel_desc_t const* const  v_desc = vehicle_builder_t::vehikel_search(wt, welt->get_current_month(), 50, welt->get_average_speed(wt), warenbauer_t::passagiere, false, true);
