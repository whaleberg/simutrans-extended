	...
	if(++ check_waiting == 0)
	{
		...
		for(uint16 j = 0; j < warenbauer_t::get_max_catg_index(); j ++)
		{
			...
			for(uint32 i = 0; i < warray->get_count(); i++)
			{
				...
				if(tmp.get_desc()->get_speed_bonus() > 0u)
				{
...
{
	w.menge = 0;
	vector_tpl<ware_t> *warray = waren[w.get_desc()->get_catg_index()];
	if(warray!=NULL) {
...
bool haltestelle_t::vereinige_waren(const ware_t &ware) //"unite were" (Google)
{
	...
	vector_tpl<ware_t> * warray = waren[ware.get_desc()->get_catg_index()];
	if(warray != NULL)
	{
		FOR(vector_tpl<ware_t>, & tmp, *warray)
		{
...
void haltestelle_t::add_ware_to_halt(ware_t ware, bool from_saved)
{
	...
	if(warray==NULL)
	{
		...
		waren[ware.get_desc()->get_catg_index()] = warray;
	}
	resort_freight_info = true;
	if(!from_saved)
	{
...
void haltestelle_t::starte_mit_route(ware_t ware, koord origin_pos)
{
	...
	if (talk)
	{
		const vector_tpl<ware_t> * warray = waren[ware.get_desc()->get_catg_index()];
